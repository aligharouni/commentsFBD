...
for Ji in range(len(J_vals)):
    ...
    Boxplot = CurveBoxPlot(curves=dic_solutions_future,sample_curves=J_vals[Ji],
                           sample_repititions=1000,time=time_arr)## 100 or 10000
...
np.savetxt("./data/juul_boundary_temp.csv", results, delimiter=',') ## sample_repititions=1000
...
## Pick one of the fname python gives the input file and the col name will be added here in out file
# fname_out <- c("juul_boundary_ss100.csv","juul_boundary_ss10000.csv")
fname_in <- "./data/juul_boundary_temp.csv" ## sample_repititions=1000
fname_out <- "./data/juul_boundary1.csv" ## sample_repititions=1000
...
nmvec <- expand.grid(e=c("lwr","upr"),
    ...
    trimws()
bnds_juul_temp <- read_csv(fname_in, col_names = FALSE) %>%  setNames(nmvec)

write.table(bnds_juul_temp,fname_out,sep=",",row.names = FALSE)

# # Files previously saved:
# # data/juul_boundary1.csv: J_vals = [2, 5, 10, 20, 30, 40, 50] , sample_repititions=1000.
