...
\begin{abstract}
Studying the concept of \emph{central set} of an ensemble of epidemic curves -- in general functional data -- provides insights to summarize and estimate the uncertainty associated with the features of interest. These insights improve our understanding of the disease dynamics and result in a reliable forcasting.
It has been argued that the classical fixed-time descriptive methods may provid unreliable estimates of centrality in functional data, specially when there exist high phase shifts among the functions. The alternative approache is to use robust curve-based descriptive methods in which ranking process and visualizing the centrality are curvewise -- as opposed to pointwise in fixed-time descriptive methods. We used the \emph{functional band depth} approach in our exploration of curve-based estimation of the central set of an epidemic ensemble. We analyzed several \emph{statistical depth functions} -- which gauges the average closness of a curve to all other curves in the ensemble -- and compared the corresponding 90\% central region. We first implemented the sample-based functional band depth algorithm (FBD) with different choices of tunning parameter $J$ to compare classical $J=2$ with the \emph{ad hoc} $J=50$ (following \juul's 2021 approach). We then developed a multivariate generalization of ranking the curves by using Mahalanobis distance among features of interest. Further, we used the pairwise Euclidean distance $\ell_2$ to define the \emph{depth}. We visually compare these methods and conclude that the classical robust FBD method (with $J=2$) and the Mahalanobis distance on features provide reasonable results \ali{not sure what else can be said as our result, thoghts?}.      
\end{abstract}
